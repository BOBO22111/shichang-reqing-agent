...
| **binance-skills-hub 的 Web3 组（13 个技能，如 `crypto-market-rank` 全市场热度榜、`binance-trading-signal` 信号流）** | 功能上与本作品最接近的是 `crypto-market-rank`（全市场社交热度 / 涨幅榜 / 聪明钱净流入）。**该组技能的主力接口挂在 `web3.binance.com`，本机实测不可达**：本机 DNS 把它解析到错误 IP（`199.59.148.97`），改用公共 DNS 取到的真实 IP（AWS 东京 `13.226.69.40`）直连时 TLS 握手被重置（`ECONNRESET`，约 50ms）→ 装了也取不到数。**例外（已实测跑通）**：`query-token-info` 的 `kline` 命令走另一个域名（`dquery.sintral.io`），本机真取到了 K 线数据（284ms）。但它属**链上单币**口径，与本作品**中心化合约全市场横截面**口径不同，混用会污染统计，故未接入 | ⛔ 主力域名本机不可达；1 个技能的命令可用但口径不符，未接入 |
